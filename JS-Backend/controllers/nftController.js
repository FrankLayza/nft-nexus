import { analyzeNFT } from "../services/huggingFaceServices.js";

export async function analyzeNFTHandler(req, res) {
  try {
    const { collection, token_id, attributes, floor_price, total_supply } = req.body;
    ...
    if (!collection || !token_id || !Array.isArray(attributes)) {
      return res.status(400).json({
        success: false,
        message: "Invalid request: collection, token_id, and attributes are required",
      });
    }
